...
let rec target' src (dest, t) = function
  | Mov(x) when x = src && is_reg dest ->
      assert (t <> Type.TyUnit);
      assert (t <> Type.TyFloat);
      false, [dest]
...
and source' t = function
  ...
  | CallCls _ | CallDir _ -> (match t with Type.TyUnit -> [] | Type.TyFloat -> [fregs.(0)] | _ -> [regs.(0)])
  | _ -> []
...
let find' x' regenv =
  match x' with
  | V(x) -> V(find x Type.TyInt regenv)
  | c -> c
...
and g' dest cont regenv = function (* 各命令のレジスタ割り当て (caml2html: regalloc_gprime) *)
  ...
  | Mov(x) -> (Ans(Mov(find x Type.TyInt regenv)), regenv)
  | Neg(x) -> (Ans(Neg(find x Type.TyInt regenv)), regenv)
  | Add(x, y') -> (Ans(Add(find x Type.TyInt regenv, find' y' regenv)), regenv)
  | Sub(x, y') -> (Ans(Sub(find x Type.TyInt regenv, find' y' regenv)), regenv)
  | Ld(x, y', i) -> (Ans(Ld(find x Type.TyInt regenv, find' y' regenv, i)), regenv)
  | St(x, y, z', i) -> (Ans(St(find x Type.TyInt regenv, find y Type.TyInt regenv, find' z' regenv, i)), regenv)
  | FMovD(x) -> (Ans(FMovD(find x Type.TyFloat regenv)), regenv)
  | FNegD(x) -> (Ans(FNegD(find x Type.TyFloat regenv)), regenv)
  | FAddD(x, y) -> (Ans(FAddD(find x Type.TyFloat regenv, find y Type.TyFloat regenv)), regenv)
  | FSubD(x, y) -> (Ans(FSubD(find x Type.TyFloat regenv, find y Type.TyFloat regenv)), regenv)
  | FMulD(x, y) -> (Ans(FMulD(find x Type.TyFloat regenv, find y Type.TyFloat regenv)), regenv)
  | FDivD(x, y) -> (Ans(FDivD(find x Type.TyFloat regenv, find y Type.TyFloat regenv)), regenv)
  | LdDF(x, y', i) -> (Ans(LdDF(find x Type.TyInt regenv, find' y' regenv, i)), regenv)
  | StDF(x, y, z', i) -> (Ans(StDF(find x Type.TyFloat regenv, find y Type.TyInt regenv, find' z' regenv, i)), regenv)
  | IfEq(x, y', e1, e2) as exp -> g'_if dest cont regenv exp (fun e1' e2' -> IfEq(find x Type.TyInt regenv, find' y' regenv, e1', e2')) e1 e2
  | IfLE(x, y', e1, e2) as exp -> g'_if dest cont regenv exp (fun e1' e2' -> IfLE(find x Type.TyInt regenv, find' y' regenv, e1', e2')) e1 e2
  | IfGE(x, y', e1, e2) as exp -> g'_if dest cont regenv exp (fun e1' e2' -> IfGE(find x Type.TyInt regenv, find' y' regenv, e1', e2')) e1 e2
  | IfFEq(x, y, e1, e2) as exp -> g'_if dest cont regenv exp (fun e1' e2' -> IfFEq(find x Type.TyFloat regenv, find y Type.TyFloat regenv, e1', e2')) e1 e2
  | IfFLE(x, y, e1, e2) as exp -> g'_if dest cont regenv exp (fun e1' e2' -> IfFLE(find x Type.TyFloat regenv, find y Type.TyFloat regenv, e1', e2')) e1 e2
  | CallCls(x, ys, zs) as exp ->
      if List.length ys > Array.length regs - 1 || List.length zs > Array.length fregs then
        ...
        g'_call dest cont regenv exp (fun ys zs -> CallCls(find x Type.TyInt regenv, ys, zs)) ys zs
...
and g'_call dest cont regenv exp constr ys zs = (* 関数呼び出しのレジスタ割り当て (caml2html: regalloc_call) *)
  (List.fold_left
     ...
     (Ans(constr
            (List.map (fun y -> find y Type.TyInt regenv) ys)
            (List.map (fun z -> find z Type.TyFloat regenv) zs)))
     (fv cont),
   M.empty)
...
let h { name = Id.L(x); args = ys; fargs = zs; body = e; ret = t } = (* 関数のレジスタ割り当て (caml2html: regalloc_h) *)
  ...
  let a =
    match t with
    | Type.TyUnit -> Id.gentmp Type.TyUnit
    | Type.TyFloat -> fregs.(0)
    | _ -> regs.(0) in
...
let f (Prog(data, fundefs, e)) = (* プログラム全体のレジスタ割り当て (caml2html: regalloc_f) *)
  ...
  let e', regenv' = g (Id.gentmp Type.TyUnit, Type.TyUnit) (Ans(Nop)) M.empty e in
  Prog(data, fundefs', e')

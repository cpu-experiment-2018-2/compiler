...
let rec apply f e =
  ...
  | If (cmp, x, y, e1, e2, d) ->
      If (cmp, f x, f y, apply' f  e1, apply' f e2, d)
...
let rec closure_to_virtual' (e: Closure.t) =
  ...
  (  Ans (If (cmp, conv x, conv y, closure_to_virtual e1, closure_to_virtual e2, d)))
  ...
  | Closure (fundef) ->
    let fptr = Syntax.Var(tmp_var()) in
    let ops x = Let(fptr,Lil(fundef.label.name,tmp_debug),
        Let(Int(0),Load(Int(30), Int(0), 0, tmp_debug),
          Let(Int(0),Store(fptr, Int(30), 0, tmp_debug),
           fst( List.fold_left (
              fun (f,counter) x -> 
              ((fun y  -> 
                f(Let(Int(0),Store(Var(x), Int(30), counter, tmp_debug),
                  y))),counter+1)
            )
            ((fun x -> x),1)
            fundef.closure_fv
     ...
     in 
    ops(
      Let(Int(30), Li(List.length fundef.closure_fv + 1, tmp_debug), 
        Let(fptr, Op(Primitive Add,  [Int(30);fptr] ,tmp_debug),
          Ans(Store(fptr, Int(0), 0, tmp_debug))
      )
    ))
  ...
  (*  *)
  | AppCls (var, ys, d) ->
      to_g(Ans (CallCls (var.name, ys, d)))
  (*  *)
  | Tuple (names, d) ->
      let ( >>= ) x f = f x in
      to_g(tmp_var ()
...
let rec function_to_virtual2 (fundef: debug Closure.fundef) =
  ...
  {label= fundef.f.name; args= List.map  conv fundef.args; body = body; ret= Syntax.Var(tmp_var ()); local= 0}

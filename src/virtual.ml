...
and 'a v = Ans of 'a u | Let of var * 'a u * 'a v
...
let rec closure_to_virtual (e: debug Knormal.u) =
  match e with
  ...
  | If (EQ, x, y, e1, e2, d) -> (
    ...
    | Type.TyBool | Type.TyInt ->
        Ans (IfEq (x, y, closure_to_virtual e1, closure_to_virtual e2, d))
    | _ -> failwith "equality is only bool and int" )
  | If (LE, x, y, e1, e2, d) -> (
    match x.ty with
    | Type.TyBool | Type.TyInt ->
        Ans (IfLE (x, y, closure_to_virtual e1, closure_to_virtual e2, d))
    | _ -> failwith "equality is only bool and int" )
  | Let (var, e1, e2, d) ->

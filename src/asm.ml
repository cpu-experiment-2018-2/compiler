open Syntax open Virtual

    type iop = Mul | Add | Sub | Div type pos =
                   LL | LH | UL | UH[@ @deriving show]

                   let tmp_debug = {pos = Global}

let iop_to_str =
    function | Mul->"mul" | Add->"add" | Sub->"sub" |
    Div->"div"

    type fop =
        Mul | Add | Sub | Div let fop_to_str =
            function | Mul->"fmul" | Add->"fadd" | Sub->"fsub" |
            Div->"fdiv"
            ...
            type label =
                ...
                BEQ of label *Syntax.debug | BLE of label *Syntax.debug |
                Jump of label *Syntax.debug | SetLabel of string *Syntax.debug |
                In of 'reg * Syntax.debug | InPos of 'reg * pos * Syntax.debug |
                Out of 'reg * Syntax.debug |
                OutPos of 'reg * pos * Syntax.debug | BLR |
                BL of label *Syntax.debug

                    type var_or_reg =
                    Reg of int | Var of var

                                     let linkreg = Reg(31) let framep =
                        Reg(2) let stackp = Reg(1)
    ...
    type var_program = Syntax.var program
        ...
        let move v1 v2 d = Opi(Add, v1, v2, 0, d)
            ...
            emit_var(e
                ...
                Li(reg, x, d)->Printf.printf "\tli %%r%s,%d (* %s *)\n" reg.name
                x(Syntax.pos_to_str d.pos) |
                Op(op, rt, ra, rb, d)
                    ->Printf
                    .printf "\t%s %%r%s,%%r%s,%%r%s (* %s *)\n"(iop_to_str op)
                        rt.name ra.name rb.name(Syntax.pos_to_str d.pos) |
                Opi(op, rt, ra, offset, d)
                    ->Printf
                    .printf
                "\t%si %%r%s,%%r%s,%d (* %s *)\n"(iop_to_str op)rt.name ra.name
                ...
                SetLabel(label, d)->Printf.printf
                ...
                "\tstore %%r%s,%%r%s,%d (* %s *)\n" rt.name rs.name
                offset(Syntax.pos_to_str d.pos) |
                Cmpd(ra, rb, d)->Printf.printf
                "\tcmpd %%r%s,%%r%s(* %s *)\n" ra.name rb.name(
                    Syntax.pos_to_str d.pos) |
                BEQ(label, d)->Printf.printf
                "\tbeq %s (* %s *)\n" label(Syntax.pos_to_str d.pos) |
                BLE(label, d)->Printf.printf
                "\tble %s (* %s *)\n" label(Syntax.pos_to_str d.pos) |
                Jump(label, d)->Printf.printf
                "\tjump %s (* %s *)\n" label(Syntax.pos_to_str d.pos) |
                In(rt, d)->Printf.printf
                "\tin %%r%s (* %s *)\n" rt.name(Syntax.pos_to_str d.pos) |
                Out(rt, d)->Printf.printf
                "\tout %%r%s (* %s *)\n" rt.name(Syntax.pos_to_str d.pos) |
                BLR->Printf.printf "\tblr\n" |
                BL(label, d)->Printf.printf
                "\tbl %s (* %s *)\n" label(Syntax.pos_to_str d.pos)
                    ...
                    oc(e
                       : int u) =
                    match e with |
                    Nop d->Printf.fprintf oc
                    "\tnop (* %s *)"(Syntax.pos_to_str d.pos) |
                    Li(reg, x, d)->Printf.fprintf oc
                    "\tli %%r%d,%d (* %s *)\n" reg
                    x(Syntax.pos_to_str d.pos) |
                    Op(op, rt, ra, rb, d)
                        ->Printf.fprintf oc
                    ...
                    "\t%s %%r%d,%%r%d,%%r%d (* %s *)\n"(fop_to_str op)rt ra
                    rb(Syntax.pos_to_str d.pos) |
                    FOpi(op, rt, ra, offset, d)
                        ->failwith "not implemented fopi" |
                    SetLabel(label, d)->Printf.fprintf oc
                    "%s : (* %s *)\n" label(Syntax.pos_to_str d.pos) |
                    Load(rt, rs, offset, d)
                        ->Printf.fprintf oc
                    "\tload %%r%d,%%r%d,%d (* %s *)\n" rt rs
                    ...
                    "\tstore %%r%d,%%r%d,%d (* %s *)\n" rt rs
                    offset(Syntax.pos_to_str d.pos) |
                    Cmpd(ra, rb, d)->Printf.fprintf oc
                    "\tcmpd %%r%d,%%r%d(* %s *)\n" ra
                    rb(Syntax.pos_to_str d.pos) |
                    BEQ(label, d)->Printf.fprintf oc
                    "\tbeq %s (* %s *)\n" label(Syntax.pos_to_str d.pos) |
                    BLE(label, d)->Printf.fprintf oc
                    "\tble %s (* %s *)\n" label(Syntax.pos_to_str d.pos) |
                    Jump(label, d)->Printf.fprintf oc
                    "\tjump %s (* %s *)\n" label(Syntax.pos_to_str d.pos) |
                    In(rt, d)->Printf.fprintf oc
                    "\tinll %%r%d (* %s *)\n" rt(Syntax.pos_to_str d.pos) |
                    Out(rt, d)->Printf.fprintf oc
                    "\toutll %%r%d (* %s *)\n" rt(Syntax.pos_to_str d.pos) |
                    BLR->Printf.fprintf oc "\tblr\n" |
                    BL(label, d)->Printf.fprintf oc
                    "\tbl %s (* %s *)\n" label(Syntax.pos_to_str d.pos)
...
let rec conv (order: debug Virtual.u) var functions =
  ...
  | CallDir (label, args, d) ->
      ...
      let ops = List.map2 (fun x y -> Opi (Add, Var(x), Var(y), 0, d)) func.args args in
...
let emit_normal functions oc =
  ...
  let lis =
    List.map
      (fun x ->
        (SetLabel (x.label, tmp_debug) ::  (pro@ virtual_to_var x.body functions x.ret)  @  epi))
      functions
  in
...
let asm_emit p func oc =
  ...
  let var = {name= Syntax.genvar ();
debug = tmp_debug;
ty = TyInt
}
in let p = register_alloc_tmp(virtual_to_var p(func @Virtual.globals) var)
    in let _ = List.iter(emit oc) p in Printf.fprintf oc "\tend"

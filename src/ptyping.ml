...
let conv plist res =
  List.fold_right (fun ty acc -> TyFun ([ty], acc)) plist res

let rec ty_subst (sigma: (int * Type.t) list) t =
  match t with
  | (TyInt | TyUnit | TyBool | TyFloat) as self -> self
  | TyFun ([a], b) -> TyFun ([ty_subst sigma a], ty_subst sigma b)
  | TyVar alpha ->
      let rec find l =
        match l with
        | [] -> TyVar alpha
...
let rec (ty_unify: type_equation list -> (int * Type.t) list) =
 fun equations ->
  ...
  | eq :: rest ->
    ...
    | TyFun ([s], t), TyFun ([s'], t') ->
        ty_unify (get_eq t t' eq.debug :: get_eq s s' eq.debug :: rest)
    | TyTuple s, TyTuple s' ->
...
let instanciate_builtin name varenv =
  ...
  | "create_array" ->
      ...
      TyFun ([TyInt], TyFun ([TyVar s], TyArray (TyVar s)))
  | "Obj.magic" ->
      let s = Type.genvar () in
...
let rec gather_eq' type_env e =
  ...
  | Op (Projection (idx, all, ty), [tup], d) -> (
      ...
      | _ -> raise (TypingError "type error tuple ") )
  ...
  | App (e1, e2, d) ->
      ...
      (alpha, get_eq (conv ts alpha) t1 d :: (c1 @ cs))

let rec last s acc =
  match s with [x] -> (acc, x) | x :: y -> last y (acc @ [x])

let rec extend s =
  match s with
  | Const _ -> s
  | Op (op, xs, d) -> Op (op, List.map extend xs, d)
  | If (t0, t1, t2, d) -> If (extend t0, extend t1, extend t2, d)
  | Let (var, t0, t1, d) -> Let (var, extend t0, extend t1, d)
  | Var (var, d) -> Var (var, d)
  | LetRec (fd, t0, d) -> extend_fundef fd t0 d
  | App (t0, ts, d) ->
      List.fold_left (fun acc p -> App (acc, [extend p], d)) (extend t0) ts
  | Tuple (ts, d) -> Tuple (List.map extend ts, d)

and extend_fundef fd t0 d =
  let b, a = last fd.args [] in
  LetRec
    ( { fd with
        args= [a]
      ; body=
          fst
            (List.fold_left
               (fun (acc, ty) p ->
                 let (TyFun ([x], y)) = ty in
                 let v = Syntax.alpha () in
                 ( LetRec
                     ( {fd with f= {v with ty}; args= [p]; body= acc}
                     , Var (v, fd.info)
                     , fd.info )
                 , y ) )
               (extend fd.body, fd.f.ty)
               b) }
    , extend t0
    , d )

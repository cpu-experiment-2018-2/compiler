...
let add env var = (var.name, "n" ^ Syntax.genvar () ^ "." ^ var.name) :: env
...
let rec (g: (string * string) list -> Knormal.t -> Knormal.t) =
 fun env e ->
  ...
  | LetRec (fundef, t1, d) ->
      ...
      let newenv = adds newenv fundef.args in
      let fundef =
        { fundef with
          body= g newenv fundef.body
        ; f= conv newenv fundef.f
        ; args= List.map (conv newenv) fundef.args }
      in
      LetRec (fundef, t1, d)

...
let get_int_value t =
  match t with Arr (x, _, _) | Tup (x, _) | Int x -> x | Float x -> getint x
...
let rec alloc x =
  ...
  | Op (ArrayGet _, [x; y], d) ->
      let (Arr (x, init, _)) = VarMap.find x !res in
      init
  | Let (name, u, v, d) ->
      let u = alloc u in
...
let f offset x =
  ...
  let ans =
    VarMap.filter
      (fun key v -> match v with Arr _ | Tup _ -> true | _ -> false)
      !res
  in
  let binded =
    VarMap.fold (fun key v acc -> VarSet.add key acc) !res VarSet.empty
  in
  let _ = Typing.binded := binded in
  let _ =
    VarMap.iter (fun key v -> Printf.printf "%s %s\n" key.name (showf v)) ans
  in
  (!hp, ans)

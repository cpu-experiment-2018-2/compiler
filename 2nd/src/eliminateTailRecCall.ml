...
let is_tailrec' fd x = 
    ...
    and sub_v x cnt =
        match x with
        | Ans(u) -> sub_u u cnt
        | Let(y,t1,u) -> sub_v u cnt
    in 
    sub_v x 0

let is_tailrec fd = 
    is_tailrec' fd fd.body_ir

let get_update_pair fd = 
    let rec get_args_u = 
        function
        | Call(f,args) -> if f.name = fd.f_ir.name  then args else failwith "unreachble" 
        | If(_,_,_,u,v,ty) -> if is_tailrec' fd u then get_args_v u else get_args_v v
...
let convert fd =
    if is_tailrec fd then 
        (
        let _ = Printf.printf "[eliminateTailRecCall.ml] found while %s\n" fd.f_ir.name in
    ...
    and change_u x = 
        match x with
        | If(cmp,x,y,u,v,ty) ->
                if is_tailrec' fd u then
                    While (cmp,ty,x,y, pair,flatten u,v)

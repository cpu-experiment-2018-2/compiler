...
let rec used = function
  ...
  | Let (var, e1, e2, _) -> VarSet.union (used e1) (used e2)
...
let rec remove tuple_env array_env check e =
  ...
  | Let (v, e1, e2, d) -> (
    ...
    | Tuple (l, d) when check v ->
        let _ = Printf.printf "%s is erased[tuple]\n" v.name in
    ...
    | AppDir ({name= "create_array"}, [_; init], _)
     ...
     |AppDir ({name= "Array.create"}, [_; init], _)
      when check v ->
        let _ = Printf.printf "%s is erased[array]\n" v.name in
        remove tuple_env (VarMap.add v init array_env) check e2
    | _ -> Let (v, g e1, g e2, d) )
  | If (cmp, x, y, e1, e2, d) -> If (cmp, x, y, e1, e2, d)
  | Op (ArrayGet _, [x; _], d) when VarMap.mem x array_env ->
      Var (VarMap.find x array_env, d)

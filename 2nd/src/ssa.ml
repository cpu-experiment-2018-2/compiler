...
type name = IR.var [@@deriving show]
...
let print_u = function
  ...
  | SOp (op, names) ->
      Printf.printf "%s" (Syntax.show_op op) ;
      List.iter (fun x -> Printf.printf "%s " x.name) names
  | SLoad (x, y) ->
      Printf.printf "LOAD " ;
      List.iter (fun x -> Printf.printf "%s " x.name) [x; y]
  | SStore (x, y, z) ->
      Printf.printf "STORE " ;
      List.iter (fun x -> Printf.printf "%s " x.name) [x; y; z]
  | SPhi x ->
      Printf.printf "phi " ;
      List.iter (fun (x, y) -> Printf.printf "[%s,%s]" x y.name) x
  | SCallDir (f, names) ->
      Printf.printf "CALL %s " f.name ;
      List.iter (fun x -> Printf.printf "%s " x.name) names

let print_v = function
  | SRet x -> Printf.printf "RET %s" x.name
  | SBr (cmp, x, y, e1, e2, ty) ->
      Printf.printf "SBr %s %s %s %s" x.name y.name e1 e2
    ...
    | Dest n ->
        ...
        let _ =
          blocks :=
            ( !current_label
            , List.rev !buf
            , SBr (c, x, y, true_block, false_block, ty) )
            :: !blocks
        in
        let _ = buf := [] in
        ...
        let _ =
          blocks :=
            (!current_label, List.rev !buf, SJump cont_block) :: !blocks ;
          clear ()
        in
